...
const PROJECT_CATEGORY_ALIASES: Record<string, ProjectCategory> = {
...
function normalizeProjectCategory(category?: string): ProjectCategory {
	...
	if ((PROJECT_CATEGORIES_ORDER as readonly string[]).includes(category)) {
		return category as ProjectCategory;
	}
	return PROJECT_CATEGORY_ALIASES[category] || DEFAULT_PROJECT_CATEGORY;
}
