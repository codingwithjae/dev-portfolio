	...
	return (
		<div className="faq-item group py-[0.9375rem] mx-[0.9375rem] border-b border-border-base transition-colors duration-300">
			...
			>
				...
				>
					<div
						className={`relative w-3 h-3 transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? "rotate-180" : ""}`}
					>
						{/* Horizontal line */}
						<div
							className={`absolute top-1/2 left-0 w-full h-[3px] -translate-y-1/2 rounded-full transition-colors duration-300 ${isOpen ? "bg-black" : "bg-text-base"}`}
						></div>
						{/* Vertical line (hides when open) */}
						<div
							className={`absolute top-0 left-1/2 w-[3px] h-full -translate-x-1/2 rounded-full transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? "rotate-90 scale-0 bg-black" : "bg-text-base"}`}
						></div>
					</div>
				</span>
			</button>
